...
def transmittans(bvf):
    red_wavelength = 607 # Replace with wavelength in nanometres
    ...
    pendepth = 1/np.sqrt(3*(musr+mua)*mua)
    print(pendepth)

    d = 300*10**(-6) #meter
    return np.exp(-np.sqrt(3*mua*(musr+mua))*d)


K = np.abs(transmittans(1)-transmittans(0.01))/transmittans(0.01)
print(K)
